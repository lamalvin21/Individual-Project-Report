% maintainence: Depends on sparql, depends on sparql anything tool, dataset up to date, json format redundant. 
...
\hspace*{0.5cm} ``Using the executable sparql-anything-0.8.1.jar, run the query in filename.sparql from the filepath folder and pass in \textit{variable} into the query with value variablevalue. Finally, output the knowledge graph in outputfile.ttl from the same file location". 
...
\hspace*{0.5cm} This branch of technical data regarding an organ was mainly derived off one .JSON file and a section of the provided ontology that referred to its 'parthood'. All relationships branching from this 'technicals' node were provided. Relevant data and their relationships identified are below:
...
\hspace*{0.5cm} This subsection will detail the process for creating the query that is responsible for extracting data from the dataset using \textit{Figure 7.3} and the query skeleton structure in the design section. All code in this section will be part of the query's WHERE clause. Below is an example of data abstraction from a .JSON file:
...
The snippet above follows \textit{Figure 7.3} and provides the template for most SPARQL-Anything SERVICE calls in the query. The query's sequence of events will be numbered below:
